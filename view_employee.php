...
<div class="contriner-fluid">
	<div class="col-md-12">
		<h5><b><small>ID empleado :</small><?php echo $employee_no ?></b></h5>
		<h4><b><small>Nombre: </small><?php echo ucwords($lastname.", ".$firstname." ",$middlename) ?></b></h4>
		<p><b>Proyecto : <?php echo ucwords($dname) ?></b></p>
		<p><b>Cargo : <?php echo ucwords($pname) ?></b></p>
		<hr class="divider">
		<div class="row">
			<div class="col-md-6">
				<div class="card">
					...
					<div class="card-body">
						<ul class="list-group">
						  ...
						  <li class="list-group-item d-flex justify-content-between align-items-center alist" data-id="<?php echo $row['id'] ?>">
						  	<span>
						    <p><small><?php echo $row['aname'] ?> Novedades</small></p>
						    <p><small>Tipo: <?php echo $t_arr[$row['type']] ?></small></p>
						    <?php if($row['type'] == 3): ?>
						    <p><small>Fecha elaboracion: <?php echo date("M d,Y",strtotime($row['effective_date'])) ?></small></p>
						    <?php endif; ?>
						    </span>
...
function remove_allowance(id){
			...
			$.ajax({
				...
				success:function(resp){
						if(resp == 1){
							alert_toast("Novedad exitosamente eliminada","success");
		...
		$('.remove_deduction').click(function(){
				_conf("Estas seguro de eliminar esta deduccion?","remove_deduction",[$(this).attr('data-id')])
			})
function remove_deduction(id){
			...
			$.ajax({
				...
				success:function(resp){
						if(resp == 1){
							alert_toast("Deduccion fue eliminada exitosamente","success");
